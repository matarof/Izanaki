...
SurfaceHolder.Callback, SpectrumDraw {
	...
	private void doSpectrumDraw(Canvas canvas, double buffer1[], int bufSize1, double buffer2[], int bufSize2, int peakIndex, double ra, float freq, String note){
		...
		for(int i=0; i*2<bufSize2; i++){
						
			int y1 = (int)(Math.log10(buffer1[i])*20);	
			canvas.drawLine(i*8, 470, i*8, 470-y1, paint);  //PowerSpectrum描画
			
			int y2 =(int)(buffer2[i*2]/1.0E4);  
			canvas.drawLine(i, 100, i, 100-y2, paint);  //autocorr描画
		...
		if(peakIndex != 0){
			canvas.drawLine(peakIndex/2, 0, peakIndex/2, 200, paint);
			canvas.drawLine(freq/(44100/fftsize)*8, 320, freq/(44100/fftsize)*8, 470, paint);
		}
